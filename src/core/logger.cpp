...
bool Logger::initialize(const std::string& component_name, const LogConfig& config) {
    ...
    if (!pimpl_->config_.real_time_flush) {
        pimpl_->flush_thread_ = std::make_unique<std::thread>(&Impl::flush_loop, pimpl_.get());
    }
...
void Logger::log_trace(const std::string& message, const char* file_name, int line_number, const char* function_name) {
    log_message(LogLevel::TRACE, message, "", file_name, line_number, function_name);
}

void Logger::log_debug(const std::string& message, const char* file_name, int line_number, const char* function_name) {
    log_message(LogLevel::DEBUG, message, "", file_name, line_number, function_name);
}

void Logger::log_info(const std::string& message, const char* file_name, int line_number, const char* function_name) {
    log_message(LogLevel::INFO, message, "", file_name, line_number, function_name);
}

void Logger::log_warning(const std::string& message, const char* file_name, int line_number, const char* function_name) {
    log_message(LogLevel::WARNING, message, "", file_name, line_number, function_name);
}

void Logger::log_error(const std::string& message, const char* file_name, int line_number, const char* function_name) {
    log_message(LogLevel::ERROR, message, "", file_name, line_number, function_name);
}

void Logger::log_critical(const std::string& message, const std::string& safety_context, const char* file_name, int line_number, const char* function_name) {
    log_message(LogLevel::CRITICAL, message, safety_context, file_name, line_number, function_name, true);
}

void Logger::log_fatal(const std::string& message, const std::string& safety_context, const char* file_name, int line_number, const char* function_name) {
    log_message(LogLevel::FATAL, message, safety_context, file_name, line_number, function_name, true);
}

void Logger::log_message(LogLevel level, const std::string& message, const std::string& safety_context, 
                        const char* file_name, int line_number, const char* function_name, bool is_safety_critical) {
...
LogLevel Logger::get_log_level() const {
    return pimpl_->current_level_.load();
}
...
LogLevel string_to_log_level(const std::string& level_str) {
    ...
    if (level_str == "WARN" || level_str == "WARNING") return LogLevel::WARNING;
...
std::string calculate_checksum(const LogEntry& entry) {
    ...
    std::ostringstream oss;
    oss << entry.sequence_number << static_cast<int>(entry.level) << entry.message;
