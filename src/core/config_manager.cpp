...
public:
    ...
    bool validate_parameter(const std::string& name, const std::string& value) const {
...
std::string ConfigManager::get_string(const std::string& name, const std::string& default_value) const {
    std::lock_guard<std::mutex> lock(pimpl_->config_mutex_);
...
double ConfigManager::get_double(const std::string& name, double default_value) const {
    std::string str_value = get_string(name);
    if (str_value.empty()) {
        return default_value;
    }
...
bool ConfigManager::get_bool(const std::string& name, bool default_value) const {
    std::string str_value = get_string(name);
    if (str_value.empty()) {
        return default_value;
    }
...
std::chrono::milliseconds ConfigManager::get_duration(
...
bool ConfigManager::set_string(const std::string& name, const std::string& value) {
    std::lock_guard<std::mutex> lock(pimpl_->config_mutex_);
...
void ConfigManager::register_validation_callback(ConfigValidationCallback callback) {
    if (callback) {
        std::lock_guard<std::mutex> lock(pimpl_->config_mutex_);
...
std::vector<ConfigParameter> create_default_safety_parameters() {
    ...
    params.push_back({
        "safety.fault_injection.max_rate",
        ConfigType::DOUBLE,
        "Maximum fault injection rate",
        true,
        true,
        "0.1",
        "0.0",
