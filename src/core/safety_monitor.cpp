...
SafetyResult
SafetyMonitor::check_constraint(const std::string &constraint_name) {
  std::lock_guard<std::mutex> lock(pimpl_->constraints_mutex_);
...
SafetyResult
SafetyMonitor::check_scenario_safety(const std::string &scenario_content) {
  ...
  std::vector<std::string> dangerous_keywords = {
      "emergency_stop", "critical_fault", "patient_disconnect",
      "power_failure"};
...
SafetyStatus SafetyMonitor::get_safety_status() const {
  ...
  status.critical_violations = std::count_if(
      pimpl_->recent_violations_.begin(), pimpl_->recent_violations_.end(),
  ...
  status.recent_violations.assign(pimpl_->recent_violations_.end() - max_recent,
                                  pimpl_->recent_violations_.end());

  // Calculate timing statistics
  if (pimpl_->total_checks_ > 0) {
    status.avg_check_duration =
        pimpl_->total_check_duration_ / pimpl_->total_checks_;
  }
...
SafetyMonitor::get_recent_violations(size_t max_count) const {
  ...
  if (count > 0) {
    result.assign(pimpl_->recent_violations_.end() - count,
                  pimpl_->recent_violations_.end());
  }
...
bool SafetyMonitor::is_system_safe() const {
  SafetyResult result =
      const_cast<SafetyMonitor *>(this)->check_system_safety();
...
bool SafetyMonitor::set_constraint_enabled(const std::string &constraint_name,
                                           bool enabled) {
  std::lock_guard<std::mutex> lock(pimpl_->constraints_mutex_);
...
bool SafetyMonitor::update_constraint_interval(
    const std::string &constraint_name, std::chrono::milliseconds interval) {
...
std::string SafetyMonitor::generate_safety_report() const {
  ...
  report << "Monitoring Active: "
         << (status.is_monitoring_active ? "Yes" : "No") << "\n";
  ...
  report << "Average Check Duration: " << status.avg_check_duration.count()
         << "ms\n";

  if (!status.recent_violations.empty()) {
    report << "\nRecent Violations:\n";
    for (const auto &violation : status.recent_violations) {
      report << "- " << violation.constraint_name << ": "
  ...
  while (is_monitoring_) {
...

    {
      ...
      for (const auto &[name, constraint] : constraints_) {
        if (!is_monitoring_)
          break; // Exit if monitoring stopped
        ...
        if (result >= SafetyResult::VIOLATION) {
          ...
          violation.requires_emergency_stop =
              (result == SafetyResult::CRITICAL_VIOLATION);
...
SafetyResult SafetyMonitor::Impl::check_constraint_internal(
...
std::string safety_result_to_string(SafetyResult result) {
  ...
  case SafetyResult::WARNING:
    return "WARNING";
  case SafetyResult::VIOLATION:
    return "VIOLATION";
  case SafetyResult::CRITICAL_VIOLATION:
    return "CRITICAL_VIOLATION";
  case SafetyResult::SYSTEM_FAILURE:
...
std::string constraint_type_to_string(SafetyConstraintType type) {
  switch (type) {
  case SafetyConstraintType::TIMING_CONSTRAINT:
    return "TIMING_CONSTRAINT";
  case SafetyConstraintType::RESOURCE_CONSTRAINT:
    return "RESOURCE_CONSTRAINT";
  case SafetyConstraintType::SIGNAL_CONSTRAINT:
    return "SIGNAL_CONSTRAINT";
  case SafetyConstraintType::COMMUNICATION_CONSTRAINT:
    return "COMMUNICATION_CONSTRAINT";
  case SafetyConstraintType::PATIENT_SAFETY:
    return "PATIENT_SAFETY";
  case SafetyConstraintType::SYSTEM_INTEGRITY:
...
std::vector<SafetyConstraint> create_default_bci_constraints() {
  ...
  timing_constraint.description =
      "Real-time response constraint for BCI commands";
...
bool validate_safety_constraint(const SafetyConstraint &constraint) {
  ...
  if (constraint.description.empty())
    return false;
  if (constraint.check_interval < std::chrono::milliseconds(1))
    return false;
  if (constraint.violation_timeout < std::chrono::milliseconds(1))
    return false;
