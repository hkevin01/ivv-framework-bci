...
struct VerifierConfig {
    ...
    std::vector<std::string> critical_functions; ///< List of safety-critical functions
};
 ...
 */
using SafetyAssertionCallback = std::function<bool(const std::string& assertion_name)>;
...
public:
     ...
     */
    virtual VerificationReport execute_scenario(const std::string& scenario_file) = 0;
     ...
     */
    virtual VerificationReport execute_scenario_content(const std::string& scenario_content) = 0;
     ...
     */
    virtual void register_safety_assertion(
